...
public class VoiceIndicatorView extends View {
    ...
    public VoiceIndicatorView(Context context, AttributeSet attrs) {
        super(context, attrs);
        //取出自定义属性
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.voiceIndicatorAttr);
        pointerColor = ta.getColor(R.styleable.voiceIndicatorAttr_pointer_color, Color.RED);
        pointerNum = ta.getInt(R.styleable.voiceIndicatorAttr_pointer_num, 4);//指针的数量，默认为4
        pointerWidth = ta.getFloat(R.styleable.voiceIndicatorAttr_pointer_width, 5f);//指针的宽度，默认5dp
        pointerSpeed = ta.getInt(R.styleable.voiceIndicatorAttr_pointer_speed, 40);
        init();
    }
    ...
    public VoiceIndicatorView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.voiceIndicatorAttr);
        pointerColor = ta.getColor(R.styleable.voiceIndicatorAttr_pointer_color, Color.RED);
        pointerNum = ta.getInt(R.styleable.voiceIndicatorAttr_pointer_num, 4);
        pointerWidth = ta.getFloat(R.styleable.voiceIndicatorAttr_pointer_width, 5f);
        pointerSpeed = ta.getInt(R.styleable.voiceIndicatorAttr_pointer_speed, 40);
        init();
    }
