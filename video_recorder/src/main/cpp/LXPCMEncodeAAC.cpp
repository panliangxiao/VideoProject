...
LXPCMEncodeAAC::LXPCMEncodeAAC(LXMediaArguments* arg):arguments(arg){
 ...
 * @return
 */
int LXPCMEncodeAAC::flushEncoder(AVFormatContext *fmt_ctx, unsigned int stream_index) {
    int ret;
    int got_frame;
    AVPacket enc_pkt;
 ...
 * @return
 */
int LXPCMEncodeAAC::initAudioEncoder() {
...
//    av_dict_set(&param, "profile", "aac_he", 0);
    ...
    is_end=START_STATE;
    pthread_t thread;
    pthread_create(&thread, NULL, LXPCMEncodeAAC::startEncode, this);
 ...
 * 用户结束标记
 */
void LXPCMEncodeAAC::userEnd(){
    is_end=END_STATE;
}

void LXPCMEncodeAAC::release(){
    is_release=LX_TRUE;
}
/**
 * 发送一帧到编码队列
 ...
 * @return
 */
int LXPCMEncodeAAC::sendOneFrame(uint8_t* buf){
...
//        av_free(frame_buf);
    ...
    arguments->handler->setAudioState(END_STATE);
 ...
 * @return
 */
void * LXPCMEncodeAAC::startEncode(void* obj) {
    LXPCMEncodeAAC *aac_encoder = (LXPCMEncodeAAC *)obj;
