...
LXYUVEncodeH264::LXYUVEncodeH264(LXMediaArguments *arg) : arguments(arg) {
 ...
 * @return
 */
int LXYUVEncodeH264::flushEncoder(AVFormatContext *fmt_ctx, unsigned int stream_index) {
    int ret;
    int got_frame;
    AVPacket enc_pkt;
 ...
 * @return
 */
int LXYUVEncodeH264::initVideoEncoder() {
...
//        av_dict_set(&param, "profile", "baseline", 0);
    ...
    is_end = START_STATE;
    pthread_t thread;
    pthread_create(&thread, NULL, LXYUVEncodeH264::startEncode, this);
 ...
 * @return
 */
int LXYUVEncodeH264::startSendOneFrame(uint8_t *buf) {
 ...
 * @return
 */
void *LXYUVEncodeH264::startEncode(void *obj) {
    LXYUVEncodeH264 *h264_encoder = (LXYUVEncodeH264 *) obj;
    while (!h264_encoder->is_end||!h264_encoder->frame_queue.empty()) {
        ...
        h264_encoder->customFilter(h264_encoder, picture_buf, in_y_size,
 ...
 */
void
LXYUVEncodeH264::customFilter(const LXYUVEncodeH264 *h264_encoder, const uint8_t *picture_buf,
 ...
 * @return
 */
int LXYUVEncodeH264::encodeEnd() {
    //Flush Encoder
    int ret_1 = flushEncoder(pFormatCtx, 0);
    if (ret_1 < 0) {
...
//        av_free(picture_buf);
    ...
    arguments->handler->setVideoState(END_STATE);
    arguments->handler->tryEncodeOver(arguments);
    return 1;
}
 ...
 * 用户中断
 */
void LXYUVEncodeH264::userEnd() {
    is_end = END_STATE;
}
void LXYUVEncodeH264::release()  {
    is_release = LX_TRUE;
}